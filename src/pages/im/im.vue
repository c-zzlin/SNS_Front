...
Alldata
		...
		query_myfriend:function(){                      //获取好友列表
		    this.$http.post('/user/query_myfriend',{uid:uid},{emulateJson:true}).then(function(res){
...
,
		...
		send(){
		    ...
		    this.reset();//清空输入框
		    var time=this.systemTime()       //获取当前时间
		    Alldata.msg.push({'pmcontent':msg,'ruid':to,'uid':from,'pmdatetime':time});               //将消息存入消息列表
